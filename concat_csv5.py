...
#This version allows the user to choose where to save the resulting file and its name
#13-Jan-2022, Lucie Kalvodova 
...
def merge_files(files_to_merge): 
	try: 
		...
		#myResult=pd.concat(allFiles, ignore_index=True, sort=False) I was trying to get rid of the added index column but it seems that the critical part is to set index=False when saving the csv
		myResult=pd.concat(allFiles)
		
		#myResult.to_csv("myMergedFile.csv", index=False)
		
		f = fd.asksaveasfile(mode='wb',  defaultextension=".csv" ) #I don't understand why using w mode causes a blank line to be added after each line, but wb works fine
		
		myResult.to_csv(f, index=False)
